...
@dp.callback_query_handler(Regexp(regexp='-get_dayShedule_'))
async def get_dayShedule(callback: types.CallbackQuery):
    if callback.data == "-get_dayShedule_1":
        course, faculty = func.get_course_and_faculty(callback)
        kbDay = types.InlineKeyboardMarkup()
        kbDay.add(types.InlineKeyboardButton("Понедельник", callback_data=f'day = 1'))
        await callback.message.answer(text="Выберите день", reply_markup=kbDay)
        kb = types.InlineKeyboardMarkup()
        for i in await parsing.get_groups(course, faculty):
            kb.add(types.InlineKeyboardButton(i["Name"], callback_data=f'{1}-get_dayShedule{i["ID"]}'))
        await callback.message.answer(text="Выберите группу", reply_markup=kb)
    else:
        await callback.message.delete()
        i = await parsing.get_day(callback.data.split("_")[-1], callback.data[1])
        await callback.message.answer(text=str(i), parse_mode="html")
    await callback.answer()
